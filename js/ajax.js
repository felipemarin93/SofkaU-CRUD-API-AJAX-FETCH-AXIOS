...
*archivo.js que contiene la lógica del crud en ajax para comunicar con la api
*/
...
const d = document,
    ...
    const getAll = () => {
      ajax({
        url: "http://localhost:3000/sofkianos",
        success: (res) => {
          console.log(res);
    ...
    d.addEventListener("submit", e => {
      if (e.target === $form) {
        ...
        if (!e.target.id.value) {
          ...
          //Update - PUT
          ajax({
            url: `http://localhost:3000/sofkianos/${e.target.id.value}`,
            method: "PUT",
            success: (res) => location.reload(),
            error: (err) => $form.insertAdjacentHTML("afterend", `<p><b>${err}</b></p>`),
            data: {
    ...
    d.addEventListener("click", e => {
      ...
      if (e.target.matches(".delete")) {
        ...
        if (isDelete) {
          //Delete - DELETE
          ajax({
            url: `http://localhost:3000/sofkianos/${e.target.dataset.id}`,
            method: "DELETE",
            success: (res) => location.reload(),
            error: (err) => alert(err)
          });
